...
logical:: diffuse_microphysics_prognostics = .true. ! Added March 14 2022 by ARR
! Local diagnostics:
...
subroutine sgs_setparm()
  ...
  !======================================================================
  !ARR added diffuse_microphysics prgonostics on March 14 2022 
  NAMELIST /SGS_TKE_ARR/ & 
       dosmagor, &! Diagnostic Smagorinsky closure
       diffuse_microphysics_prognostics
  ...
  read (55,SGS_TKE_ARR,IOSTAT=ios)
  ...
  if (ios.ne.0) then
     ...
     if(ios.ne.ios_missing_namelist) then
        write(*,*) '****** ERROR: bad specification in SGS_TKE_ARR namelist'
        call task_abort()
     end if
  end if
  close(55)
   ...
   if(masterproc) then
      ...
      write (unit=55,nml=SGS_TKE_ARR,IOSTAT=ios)
      write(55,*) ' '
      close(unit=55)
   end if
...
!
      ...
      !ARR March 14 2022 added diffuse_microphysics_prognostics
      if(diffuse_microphysics_prognostics) then
         !write(*,*) '****** I AM DIFFUSING WATER VAPOR! ********'
         call micro_flux()
